...
class ShootCommands(commands.Cog):
    ...
    async def assignRole(self, interaction: discord.Interaction, user: discord.Member):
        logger.info(f"{interaction.user.mention} wants to shoot {user.mention}")
        ...
        logger.info(f"User has role {role.name}: {has_role}")
        ...
        try:
            logger.info(f"Checking timeout status for {user.mention}")
            if user.timed_out:
                await interaction.response.send_message(f"{user.mention} is timed out, they will not be shot.", ephemeral=True)
                return
        except AttributeError:
            logger.warning(f"Cannot check timeout for {user.mention}, skipping this check.")

        logger.info(f"User has gun: {True}")  # Replace this with your actual logic
        ...
        logger.info(f"User has vest: {False}")  # Replace this with actual vest logic
        ...
        try:
            logger.info(f"Assigning shot role and timeout to {user.mention}")
